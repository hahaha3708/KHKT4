...
def Signin(event=None):
    ...
    if USERNAME.get() == "" or PASSWORD.get() == "":
        ...
        conn.commit()
        lbl_text.config(text="Sign in successfully", fg="red")
        USERNAME.set("")
        PASSWORD.set("")
    cursor.close()
    conn.close()
...
btn_login.grid(pady=25, row=3, columnspan=4)
btn_login.bind('<Return>', Login)

btn_signin = Button(Form, text="Sign in", width=45, command=Signin)
btn_signin.grid(pady=25, row=3, columnspan=3)
btn_signin.bind('<Return>', Signin)
